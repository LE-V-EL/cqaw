...
for data_class, mem in Dataclass.__members__.items():
    ...
    for i in range(len(TRAIN_DATA)):
        ...
        im = np.vstack((td[0], np.ones(td[0].shape)*200))
        ...
        if lev<3:
            ...
            cv2.putText(im, SIMPLE_QUERIES[mem.value], (20, 600), font, 0.35, 0)
            cv2.putText(im, str(td[1]), (20, 650), font, textscale, 0)
        elif lev==3:
            ...
            cv2.putText(im, train_queries[i], (20, 600), font, 0.35, 0)
            cv2.putText(im, str(train_labels[i]), (20, 650), font, textscale, 0)
        train_metadata = train_metadata + [mtdt]


        cv2.putText(im, im_filename, (20, 450), font, textscale, 0)
        cv2.putText(im, data_class.lower(), (20, 500), font, textscale, 0)
        cv2.putText(im, str(lev), (20, 550), font, textscale, 0)
        cv2.imwrite(path+im_filename, im)
    ...
    for i in range(len(TEST_DATA)): 
        ...
        if lev<3:
            ...
            amtdt = { # Only the admin metadata has the true label
                ...
                "label":test_labels[i]} 
            cv2.putText(im, test_queries[i], (20, 600), font, 0.35, 0)
            cv2.putText(im, str(test_labels[i]), (20, 650), font, textscale, 0)
    ...
    with open('./sanity_check_fig/TRAIN_metadata.txt', 'w') as outfile:
        json.dump(train_metadata, outfile, default=convert)
    with open('./sanity_check_fig/TEST_metadata.txt', 'w') as outfile:
        json.dump(test_metadata, outfile, default=convert)
    with open('./sanity_check_fig/ADMIN_metadata.txt', 'w') as outfile: 
